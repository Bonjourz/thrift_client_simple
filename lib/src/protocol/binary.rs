...
use super::{
    TFieldIdentifier, TAsyncInputProtocol, /* TInputProtocolFactory, */ TListIdentifier, TMapIdentifier,
    ...
    T: TReadTransport + Send,
{
    ...
    async fn read_message_begin(&mut self) -> crate::Result<TMessageIdentifier> {
        ...
        if first_bytes[0] >= 8 {
            ...
            if self.strict {
                ...
                let message_type_raw = self.read_byte().await?;
                let message_type: TMessageType = TryFrom::try_from(message_type_raw)?;
    ...
    async fn read_list_begin(&mut self) -> crate::Result<TListIdentifier> {
        let element_type_raw = self.read_byte().await?;
        let element_type: TType = field_type_from_u8(element_type_raw)?;
    ...
    async fn read_map_begin(&mut self) -> crate::Result<TMapIdentifier> {
        let key_typt_raw = self.read_byte().await?;
        let key_type: TType = field_type_from_u8(key_typt_raw)?;

        let value_type_raw = self.read_byte().await?;
        let value_type: TType = field_type_from_u8(value_type_raw)?;
